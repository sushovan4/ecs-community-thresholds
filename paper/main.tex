...
\title{Locating community thresholds along environmental gradients with Euler
...
plots) and holds false-positive rates near nominal. It and TITAN each miss
the other's signal: pure abundance shifts versus co-occurrence
reorganizations. Because the Euler characteristic is a sum of local
curvatures, a detected change decomposes exactly into per-taxon shares, so
the method also names the taxa that rearranged. Applied under pre-registration to eight
public gradients, it detects in one: a twenty-year Everglades insect panel,
where the five taxa carrying half the change are none of TITAN's indicator
taxa. We map the operating range that
explains the rest, including a collapse in power from 0.60 on Gaussian
abundances to 0.13 on realistic sparse counts, and a threefold
over-rejection by plot-level permutation where the gradient is spatially
structured. Python and R implementations are released.
\end{abstract}
...
recovery error, its power as a function of survey design, a negative result
...
confounding---that says where the method stops working
(Sections~\ref{sec:simgrad}--\ref{sec:limits}); (iv) evidence that the method and taxon-aggregative detection
are complementary in both directions (Section~\ref{sec:titan}); (v) an exact attribution of a detected change to individual taxa through
the discrete Gauss--Bonnet decomposition of $\chi$
(Section~\ref{sec:curvature}), which on real data names taxa disjoint from
those a taxon-level method names; (vi) pre-registered applications to eight
public gradients and ten long-term series, reported in full whether they
detect or not (Sections~\ref{sec:public}, \ref{sec:program}); and
(vii) the pre-registered analysis protocol for the Butte Hill restoration
...
\section{Two Everglades datasets: a benchmark and a detection}
\label{sec:public}
...
TITAN on the same matrix places its decliner change point at 2561 days and
its increaser change point at 338 days, so the structural change point
falls between the two taxon-level ones.
...
of TITAN's eight pure and reliable indicator taxa, and TITAN's indicators
contribute at most $1\%$ each to the structural change. The two
...
Two public transcriptomic datasets were analyzed earlier in the development
of the method, before the survey datasets of Section~\ref{sec:program} were
assembled, and we report them because the failure modes they exposed shaped
the estimator. The construction is agnostic to what the ``taxa'' are, and
co-expression gives it the matrix shape of Section~\ref{sec:data} at
realistic size and noise. Both results bound what the pipeline can claim;
neither supports it.
...
\section{The instrument on eight public gradients}
\label{sec:program}

A method's operating range is only useful if it is tested where the data
are, so we applied the frozen estimator to every public dataset we could
find that met three requirements fixed in advance: a taxa-by-unit abundance
matrix, a measured ordered gradient per unit, and at least 40 units and 15
taxa. Gradient variables were chosen from each dataset's metadata before
download, and all choices, inclusions and exclusions were registered before
any estimator ran. Table~\ref{tab:program} reports every dataset, detected
or not.

\begin{table}[t]
\centering
\caption{The frozen estimator on eight public gradients, all pre-registered.
$p$ is the registered 199-permutation test; $p_{\text{reg}}$ permutes
within regions, the test that matters where the gradient is spatially
structured (Section~\ref{sec:limits}). Everglades LT is the long-term
insect panel of Section~\ref{sec:public}; the remaining rows are
single-survey designs.}
\label{tab:program}
\small
\begin{tabular}{llrrrr}
\toprule
Dataset & Gradient & Units & $\hat z$ & $p$ & $p_{\text{reg}}$ \\
\midrule
Everglades LT insects & days since dry & 148 & 1108 d & \textbf{0.045} & \textbf{0.040} \\
EPA rivers \& streams & conductivity & 1914 & 131 & \textbf{0.015} & 0.34 \\
Niwot alpine plants & snow depth & 81 & 101 cm & 0.025$^\ast$ & --- \\
EPA streams, Appalachia & conductivity & 501 & 304 & 0.235 & 0.275 \\
EMAP mine-drainage streams & conductivity & 572 & 46.9 & 0.140 & 0.145 \\
EPA estuaries & sediment copper & 689 & 2.48 & 0.420 & 0.605 \\
EPA lakes & total phosphorus & 1085 & 25.8 & 0.650 & 0.945 \\
Cedar Creek (N addition) & N dose & 207 & 7.83 & 0.175 & 0.205 \\
\bottomrule
\end{tabular}
\par\smallskip
\footnotesize $^\ast$ the registered verdict; the same test at 1999
permutations gives $p = 0.056$, so this detection is not stable.
\end{table}

One dataset detects and survives its regional control. A second, the
national stream survey, rejects at $p = 0.015$ (stable at 1999
permutations) but not once units are permuted within ecoregions
($p = 0.34$): specific conductance varies between ecoregions for reasons
that have nothing to do with the local community, and the stress map
predicts exactly this failure mode. A third is borderline and unstable. The
remaining five do not reject.

We read this as the operating range of Section~\ref{sec:limits} asserting
itself rather than as evidence that community structure is unresponsive.
Six of the eight are sparse count matrices, where simulated power is
$0.07$--$0.37$ rather than the $0.6$ of a Gaussian design; the one robust
detection is the design with the most surveys per unit. A cross-dataset
ordering test against TITAN's change points was registered in advance and
required at least four detections; with one, it is reported here as
underpowered and not run.

\paragraph{Time as the gradient.} The same estimator was applied with the
gradient set to calendar time on ten long-term series (Everglades regions,
Cedar Creek fields, the Niwot grid), asking whether structure reorganizes
at a datable moment. None of the ten rejected. Their unit counts---20 to 35
survey years---are below the range where the test has power, so this is a
resolution limit and not evidence against temporal reorganization. Where a
programme surveys several times a year, the survey occasion is the natural
unit; the three Everglades series that this affords (143--147 occasions)
give one detection (Shark River Slough, $p = 0.045$; $0.0495$ at 1999
permutations), one marginal result ($p = 0.075$) and one null. Annual
monitoring, in other words, rarely samples time densely enough for this
instrument.
...
freely inflates false positives threefold, and a within-region permutation
