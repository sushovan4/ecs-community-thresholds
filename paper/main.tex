...
thresholds decides where remediation effort pays. The standard tools ---
threshold indicator taxa analysis (TITAN), gradient forest, breakpoints on
...
change points spread across the gradient --- no coherent community threshold
--- while on the converse simulation, an abundance shift with fixed
...
co-occurrence structure with a single integer --- its Euler characteristic ---
and ask where along the gradient that shape changes. The Euler characteristic is
...
(ii) a threshold estimator on the resulting Euler characteristic surface ---
window rule, change statistic, bootstrap interval, and permutation test ---
selected on simulated gradients with known thresholds and stated precisely
...
threshold is mixed by construction, and the simulations bear this out ---
recovery error tracks window width, not the choice of statistic. Surveys
...
$\mathbb{R}^3$ (Euclidean distance; 120 points, fifteen replicates each) ---
eight modules placed independently, and the same modules arranged on a cycle
--- have identical connectance at every grid point by construction, and the
...
Erd\H{o}s--R\'enyi expectation from $\chi$ leaves the same curve ---
the topological summary is the connectance signal reprocessed.
...
against 1.98), yet no summary clears $\alpha = 0.05$ reliably ---
arrangement-only reorganization is the hardest signal in this study, real
...
statistic $4.28$ against $3.13$; recovery RMSE $0.074$ against $0.102$) ---
a comparison we report rather than bury, because the framework is
...
\citep{anderson2001permanova}, given the \emph{oracle} split at $z^\ast$ ---
their best case, since neither localizes --- rejected in 0--2 of 10 runs on
...
pipeline's remaining validation used two public transcriptomic datasets ---
the construction is agnostic to what the ``taxa'' are, and co-expression
...
abundance matrix with plots orderable by measured soil metal concentration ---
the data shape of Section~\ref{sec:data}, with the gradient already in hand.
...
null. Because TITAN's change point derives from the marginal channel ---
independent of ours by the results of Section~\ref{sec:titan} --- anchoring
